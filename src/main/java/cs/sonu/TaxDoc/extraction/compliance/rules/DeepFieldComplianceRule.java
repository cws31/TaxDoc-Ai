...
public class DeepFieldComplianceRule implements W2ComplianceRule {
    ...
    public RuleEvaluationResult evaluate(W2ExtractionResult result) {
        ...
        // 1. Evaluate SSN (Weight: 25.0 - Critical Identifier)
        ...
        feedback.append(String.format("SSN Score: %.1f (Reason: %s) | ", ssnScore.score(), ssnScore.reasoning()));

        // 2. Evaluate EIN (Weight: 20.0 - Critical Identifier)
        ...
        feedback.append(String.format("EIN Score: %.1f (Reason: %s) | ", einScore.score(), einScore.reasoning()));

        // 3. Evaluate Box 1 Wages (Weight: 20.0 - Primary Financial Figure)
        ...
        maxPossibleWeight += 20.0;
        feedback.append(
                String.format("Box 1 Wages Score: %.1f (Reason: %s) | ", box1Score.score(), box1Score.reasoning()));

        // 4. Evaluate Box 2 Federal Tax Withheld (Weight: 15.0)
        ...
        feedback.append(
                String.format("Box 2 Tax Score: %.1f (Reason: %s) | ", box2Score.score(), box2Score.reasoning()));

        // 5. Evaluate Social Security Wages & Tax (Weight: 10.0 combined)
        ...
        // 6. Evaluate Medicare Wages & Tax (Weight: 10.0 combined)
        AdvancedFieldValidator.FieldScore box5Score = fieldValidator.evaluateMonetaryField(result.box5MedicareWages());
        ...
        // Calculate final normalized rule score percentage (0.0 to 1.0)
        double normalizedRuleScore = maxPossibleWeight > 0 ? totalWeightedScore / maxPossibleWeight : 0.0;
        boolean passed = normalizedRuleScore >= 0.85; // Pass criteria for this deep field rule block
